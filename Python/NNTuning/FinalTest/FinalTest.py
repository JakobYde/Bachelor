...
def trainNetwork(parameters, x1t, x1v, x2t, x2v, yt, yv, use_same_model, model_number):
    ...
    #cb_earlystop = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=60, verbose=0,
                                                # restore_best_weights=0
                                                 # )
    cbs = [cb_lrs]

    if not use_same_model:
        model.save_weights('weights/model_number_{}.h5'.format(model_number))

    if use_same_model:
        model.load_weights('weights/model_number_{}.h5'.format(model_number))
...
writeCsvHead(['layer1', 'layer2', 'layer3', 'layer4', 'layer5', 'layer6', 'layer7', 'val_size'])

activation_functions = ['relu', 'LeakyReLu']
activation_functions_last_dense = ['relu', 'linear']
...
while(True):
    ...
    for i in range(0, n_cv):
        ...
        mae, mse, model = trainNetwork(params, x1t, x1v, x2t, x2v, yt, yv, use_same_model=use_same_model,
                                       model_number=model_number)
        mae_list.append(np.array(mae))
        mse_list.append(np.array(mse))
        models.append(model)
        use_same_model = True
    mae_averaged = np.average(np.array(mae_list), axis=0)
    mse_averaged = np.average(np.array(mse_list), axis=0)
    temp_mse = mse_averaged[len(mse_averaged)-1]
    if temp_mse < min_mse:
        min_mse = temp_mse
        print("New min loss:", min_mse, "Parameters:", params)
