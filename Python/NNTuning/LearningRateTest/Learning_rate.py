...
def trainNetwork(parameters, x1t, x1v, x2t, x2v, yt, yv, seed):
...
model = {'learning_rate': 0.01, 'n_layers': 6, 'dense_nodes': [134, 189, 157, 220, 201, 235]}
...
for s in range(amount_of_seeds):
    ...
    for learning_rate in learning_rates:
        ...
        for i in range(0, n_cv):
            ...
            performances.append(trainNetwork(model, x1t, x1v, x2t, x2v, yt, yv, seed))
        new_loss = getPerformance(performances, window_size)
        if new_loss < min_loss:
            min_loss = new_loss
            print("New min loss:", new_loss, "Parameters:", model, "Seed: ", seed)
        print(seed)